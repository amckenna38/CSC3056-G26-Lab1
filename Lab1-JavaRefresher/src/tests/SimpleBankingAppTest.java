...
public class SimpleBankingAppTest {
	...
	public static void testWithdrawals() {
		// Pick one of the accounts and record its balance
		double beforeWithdrawalBalance = mainApp.getBalance("5495-1234");
		
		// Withdraw a hard coded amount from the account
		double withdrawalAmount = 25.00;
		
		// Now withdraw the money
		mainApp.addTransaction("5495-1234", withdrawalAmount);
		double balanceAfterWithdrawal = mainApp.getBalance("5495-1234");
		
		// Use assert
		assert beforeWithdrawalBalance - withdrawalAmount == balanceAfterWithdrawal;
		
		// Print out if test has failed or not
